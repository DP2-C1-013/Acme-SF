...
public interface ClientDashboardRepository extends AbstractRepository {
	...
	@Query("SELECT count(p) FROM ProgressLog p WHERE p.contract.client.userAccount.id = :id and p.completeness between 0 and 24.9 and p.draftMode = false")
	int totalNumProgressLogLessThan25(int id);

	@Query("SELECT count(p) FROM ProgressLog p WHERE p.contract.client.userAccount.id = :id and p.completeness between 25 and 49.9 and p.draftMode = false")
	int totalNumProgressLogLessBetween25And50(int id);

	@Query("SELECT count(p) FROM ProgressLog p WHERE p.contract.client.userAccount.id = :id and p.completeness between 50 and 74.9 and p.draftMode = false")
	int totalNumProgressLogLessBetween50And75(int id);

	@Query("SELECT count(p) FROM ProgressLog p WHERE p.contract.client.userAccount.id = :id and p.completeness > 75 and p.draftMode = false")
